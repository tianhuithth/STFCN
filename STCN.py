"""
A CNN framework uses the spatial-temporal representation data as the input
made with pytorch
"""
...
def cnn(sub_index, data_train, label_train, data_test, label_test):
    ...
    np.savetxt('./result/collected_data_' + str(
        sub_index) + '.txt', save_acc, '%.10f')
    scio.savemat('./pre_and_label/collected_data_'+str(
